...
export const getPurchases = createAsyncThunk("getPurchases", async (initData) => {
    try {
  ...
  export const getSinglePurchase = createAsyncThunk("getSinglePurchase", async (id) => {
    try {
      //Creating API Call using base url (/api/purchases/:id)
      return await axios.get(`${ENDPOINTS.PURCHASE}/${id}`).then(res => res.data)
    } catch (error) {
      //In case of error
      return error.response.data.error[0]
    }
  ...
  export const updatePurchase = createAsyncThunk("updatePurchase", async (initData) => {
    try {
      //De Structuring data
      const {id, Data} = initData
    
      //Creating API call using ENDPOINTS as Base URL (/api/purchases/:id)
      return await axios.put(`${ENDPOINTS.PURCHASE}/${id}`, Data).then(res => res.data)
    } catch (error) {
...
export const purchaseStockSlice = createSlice({
    ...
    builder.addCase(getPurchases.fulfilled, (state, actions) => {
      ...
      builder.addCase(addPurchase.fulfilled, (state, action) => {
        ...
        if(action.payload?.success === true){
          console.log("Name Missing => ", action.payload)
          //toast
      ...
      builder.addCase(updatePurchase.fulfilled, (state, action)=> {
        ...
        if(action.payload.success === true){
          ...
          const updatedPurchase = {...action.payload.updated[0], id: action.payload.updated[0]._id}
