...
export const getEmployees = createAsyncThunk("getEmployees", async (initData) => {
    try {
  ...
  export const getSingleEmployee = createAsyncThunk("getSingleEmployee", async (id) => {
    try {
      //Creating API Call using base url (/api/employees/:id)
      return await axios.get(`${ENDPOINTS.EMPLOYEE}/${id}`).then(res => res.data)
    } catch (error) {
      //In case of error
      return error.response.data.error[0]
    }
  ...
  export const updateEmployee = createAsyncThunk("updateEmployee", async (initData) => {
    try {
      //De Structuring data
      const {id, Data} = initData
    
      //Creating API call using ENDPOINTS as Base URL (/api/employees/:id)
      return await axios.put(`${ENDPOINTS.EMPLOYEE}/${id}`, Data).then(res => res.data)
    } catch (error) {
    ...
    builder.addCase(getEmployees.fulfilled, (state, actions) => {
      ...
      builder.addCase(addEmployee.fulfilled, (state, action) => {
        ...
        if(action.payload?.success === true){
          ...
          //Modifying id
          const employee = {...action.payload.employee, id: action.payload.employee._id}
      ...
      builder.addCase(updateEmployee.fulfilled, (state, action)=> {
        ...
        if(action.payload.success === true){
          ...
          if(state.data.length === 5){
            ...
            //Inserting all items except last one          
            state.data.forEach((item, index) => item.id !== employee.id && popedState.push(item))
            return {
              ...state,
              data: [employee, ...popedState ],
              errors: []
