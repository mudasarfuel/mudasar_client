...
export const getSupplierPayments = createAsyncThunk("getSupplierPayments", async (initData) => {
  try {
...
export const getSingleSupplierPayment = createAsyncThunk("getSingleSupplierPayment", async (id) => {
  try {
...
export const addSupplierPayment = createAsyncThunk("addSupplierPayment", async (Data) => {
  try {
    //Creating API call using ENDPOINTS as Base URL (/api/supplierpayments)
    return await axios
...
export const updateSupplierPayment = createAsyncThunk("updateSupplierPayment", async (initData) => {
  try {
    //De Structuring data
    const {id, Data} = initData
  
    //Creating API call using ENDPOINTS as Base URL (/api/supplierpayments/:id)
    return await axios.put(`${ENDPOINTS.SUPPLIERPAYMENT}/${id}`, Data).then(res => res.data)
  } catch (error) {
...
export const deleteSupplierPayment = createAsyncThunk("deleteSupplierPayment", async (id) => {
  try {
    //Creating API Call using ENDPOINTS as Base URL (/api/supplierpayments/:id)
    return await axios
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(addSupplierPayment.fulfilled, (state, action) => {
      ...
      if(action.payload?.success === true){
        ...
        //Modifying id
        // const supplierpayment = {...action.payload.supplierpayment, id: action.payload.supplierpayment._id}
        return {
          ...state,
          // data: [...state.data, supplierpayment],
          errors: []
        }
      }
    })
    ...
    builder.addCase(updateSupplierPayment.fulfilled, (state, action)=> {
      ...
      if(action.payload.success === true){
        ...
        const updatedSupplierPayment = {...action.payload.updated[0], id: action.payload.updated[0]._id}
        //Show Alert
        toast(action.payload.msg, {position: "top-right", type: "success"})
        return {
          ...state,
          data: state.data.map(item => 
            item.id === action.payload.updated[0]._id ? updatedSupplierPayment : item
          ),
          errors: []
        }
      }
...
export const { clearSupplierPayments } = supplierPaymentSlice.actions;
